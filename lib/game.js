...
function handleInProgress(self, events, plays) {
  ...
  if (last.inning.complete) {
    last.inning.side = plays.inning_state.toLowerCase();
    last.inning.num = plays.inning;
    last.inning.complete = false;
  }

  var idx = +last.inning.num - 1;
  var side = (last.inning.side === 'top' || last.inning.side === 'middle') ? 'top' : 'bottom';
  var innings = XML.alwaysArray(events.inning);
  if (innings[idx] && innings[idx][side]) {
  ...
  if (inning && inning.atbat) {
    ...
    atbats.forEach(function(a) {
      if (a.o === '3') {
        last.inning.complete = true;
      }
      if (last.atbat.current !== 0 && a.num > last.atbat.current) {
        last.atbat.current = plays.atbat.num;
        console.log('Now batting ' + plays.players.batter.boxname + ', ' +
          plays.players.batter.avg + 'AVG');
      }
      if (a.event_num > last.atbat.completed.event) {
        last.atbat.completed.event = a.event_num;
        last.atbat.completed.num = a.num;
        console.log(a.des);
      }
    });
  }
  ...
  if (inning && inning.action) {
    ...
    actions.forEach(function(a) {
      if (a.event_num > last.action) {
        last.action = a.event_num;
        console.log(a.event + ': ' + a.des);
      }
    });
  }

  if (plays.atbat && plays.atbat.num != last.atbat.current &&
      (last.atbat.completed.num === 0 || plays.atbat.num - last.atbat.completed.num < 2)) {
    last.atbat.current = plays.atbat.num;
    console.log('Now batting ' + plays.players.batter.boxname + ', ' +
      plays.players.batter.avg + 'AVG');
  }
