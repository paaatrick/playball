...
function loop(self) {
  ...
  .spread(function(events, plays) {
    ...
    } else if (status === 'gameOver' || status === 'final') {
      handleGameOver(self, events, plays);
    } else {
...
function handleGameOver(self, events, plays) {
  if (inningInProgress) {
    return handleInProgress(self, events, plays);
  }

  utils.getXml(self.urlRoot + '/linescore.xml')
    .then(function(linescore) {
      self.emit('gameOver', linescore);
    })
    .done();
}
